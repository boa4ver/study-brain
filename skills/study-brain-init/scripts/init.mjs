#!/usr/bin/env node
// One-command setup for a study-brain site.
//
//   node init.mjs
//   node init.mjs --notes=~/Documents/Notes --project=~/study-brain-site --include="Maths,History"
//
...
const SITE_SKILL = join(PLUGIN_ROOT, "skills", "study-brain-site");
...
/**
 * study-brain builds on obsidian-second-brain: it expects a vault that is kept
 * current, and reads the same plain-markdown notes.
 *
 * This asks rather than installing silently. Downloading and running someone
 * else's code on a machine is the user's call, not a side effect of running a
 * setup script.
 */
async function ensureSecondBrain() {
  const candidates = [
    join(homedir(), ".claude", "skills", "obsidian-second-brain"),
    join(homedir(), ".claude", "plugins", "obsidian-second-brain"),
    join(homedir(), ".claude", "plugins", "marketplaces", "obsidian-second-brain"),
  ];
  if (candidates.some((p) => existsSync(p))) {
    say("  obsidian-second-brain: found");
    return;
  }

  say("\n  study-brain requires obsidian-second-brain, which is not installed.");
  say("  It is a separate MIT project by Eugeniu Ghelbur:");
  say("  https://github.com/eugeniughelbur/obsidian-second-brain\n");

  const dest = join(homedir(), ".claude", "skills", "obsidian-second-brain");
  const ok =
    has("install-deps") ||
    (interactive && /^y/i.test(await ask("  Clone and install it now? [y/N] ", "n")));

  if (!ok) {
    say("\n  Skipped. Install it yourself before using study-brain:");
    say("    /plugin marketplace add eugeniughelbur/obsidian-second-brain");
    say("    /plugin install obsidian-second-brain\n");
    return;
  }
  ...
  const clone = spawnSync(
    "git",
    ["clone", "--depth=1", "https://github.com/eugeniughelbur/obsidian-second-brain.git", dest],
    { stdio: "inherit" }
  );
  if (clone.status !== 0) {
    say("  Clone failed - install it yourself and re-run.");
    return;
  }
  if (existsSync(join(dest, "install.sh"))) {
    spawnSync("bash", [join(dest, "install.sh")], { stdio: "inherit" });
  }
  say("  obsidian-second-brain installed.");
}
...
async function main() {
  ...
  let project = flag("project") ?? (await ask("\nWhere should the site go? [~/study-brain-site] ", "~/study-brain-site"));

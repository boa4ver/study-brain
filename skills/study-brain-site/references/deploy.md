...
`~/Library/LaunchAgents/com.study-brain.vault-sync.plist` with `RunAtLoad` and
`KeepAlive` keeps the watcher alive across restarts.

```bash
launchctl load ~/Library/LaunchAgents/com.study-brain.vault-sync.plist
launchctl unload ~/Library/LaunchAgents/com.study-brain.vault-sync.plist
```
